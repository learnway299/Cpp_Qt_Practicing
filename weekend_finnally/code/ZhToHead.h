...
void getHeadbyCH(const QString& input, QString& output){
    //qDebug()<<"before"<<input;
    std::string dest_chinese=std::string(input.toLocal8Bit());//解决转换乱码问题
    //std::cout<<"after:"<<dest_chinese<<std::endl;
    std::string out_py;
    ChineseConvertPy(dest_chinese,out_py);
    // std::cout<<"sbwbw:"<<out_py<<std::endl;
    output= QString::fromStdString(out_py);
    //qDebug()<<"jadssa"<<output;
}
